...
$sql_common = " FROM {$g5['marking_table']} AS mrk "; 
...
if ($st_date) {
    if ($st_time) {
        $where[] = " mrk_reg_dt >= '".$st_date.' '.$st_time."' ";
    }
    else {
        $where[] = " mrk_reg_dt >= '".$st_date.' 00:00:00'."' ";
    }
}
if ($en_date) {
    if ($en_time) {
        $where[] = " mrk_reg_dt <= '".$en_date.' '.$en_time."' ";
    }
    else {
        $where[] = " mrk_reg_dt <= '".$en_date.' 23:59:59'."' ";
    }
}
...
<select name="sfl" id="sfl">
    <option value="mrk_value" <?=get_selected($sfl, 'mrk_value')?>>PLC값</option>
    <option value="mrk_count" <?=get_selected($sfl, 'mrk_count')?>>카운터</option>
    <option value="dta_idx" <?=get_selected($sfl, 'work_shift')?>>측정고유값</option>
</select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $s_mod_a = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=u&mrk_idx='.$row['mrk_idx'].'">';
        $s_mod = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=u&mrk_idx='.$row['mrk_idx'].'" class="btn btn_03">수정</a>';
        $s_copy = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=c&mrk_idx='.$row['mrk_idx'].'" class="btn btn_03">복제</a>';

        echo '
			<tr tr_id="'.$i.'" style="background-color:'.$row['tr_bgcolor'].';color:'.$row['tr_color'].'">
				<td>'.$row['mrk_idx'].'</td>
				<td>'.$g5['mms'][$row['mms_idx']]['mms_name'].'</td>
				<td>'.$row['mrk_reg_dt'].'</td>
				<td>'.$row['mrk_value'].'</td>
				<td>'.$row['mrk_count'].'</td>
