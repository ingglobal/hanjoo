...
for($i = 2 ; $i <= $maxRow ; $i++) {
    ...
    $dataB[$i] = $objWorksheet->getCell('B'.$i)->getValue(); // 업체번호
    ...
    $dataN[$i] = $objWorksheet->getCell('N'.$i)->getValue(); // 발생지연
    $dataO[$i] = $objWorksheet->getCell('O'.$i)->getValue(); // 내용
    $dataO[$i] = $objWorksheet->getCell('P'.$i)->getValue(); // 비고
...
//    echo '===================================== '.$dataL[$i].'<br>';
    ...
    if( is_numeric($dataA[$i]) && is_numeric($dataB[$i]) && is_numeric($dataC[$i]) && is_numeric($dataD[$i]) ) {
        ...
        $dataM[$i] = trim( $dataM[$i] );    // 하루최대
        $dataN[$i] = trim( $dataN[$i] );    // 발생지연
        $dataO[$i] = trim( $dataO[$i] );
        $dataP[$i] = trim( $dataP[$i] );
